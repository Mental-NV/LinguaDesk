...
internal static class EvaluateRewriting
{
    ...
    {
        var (rows, mismatches) = RunOfflineSection(profile, eligibilityProbe, rewritingProbe);
        ...
        EvaluateRewritingOptions selected)
    {
        ...
        using (credential)
        {
            ...
            var (rows, failed, mismatches) = await RunLiveSectionAsync(profile, eligibilityProbe, rewritingProbe, budget, credential).ConfigureAwait(false);
            var observations = rows.Select(row => (CaseObservation)row.Observation).ToList();
            var report = new RewritingEvaluationReport(
                "rewriting_evaluation_report",
                profile.CandidateId,
        ...
        TransportCredential credential)
    {
        ...
        foreach (var kase in Cases)
        {
            ...
            catch (ChatCompletionsAdapterException exception) when (exception.Kind == AttemptFailureKind.BudgetDenied)
            {
                var denied = Observe(
                    ...
                    budget.ReservedUsd,
                    ActualUsd: null);
                rows.Add(new SectionRow(kase.CaseId, nameof(RewritingDecision.Failed), denied));
                break;
            }
        ...
        var failed = rows.Count(row =>
            string.Equals(row.Decision, nameof(RewritingDecision.Failed), StringComparison.Ordinal));
        var mismatches = rows.Count(row => !((CaseObservation)row.Observation).MatchesReference);
