...
[TestClass]
public sealed class OperationRewritingTests
{
    ...
    private static async Task<(OperationFixture Fixture, FakeRewritingProvider Fake)> CreateRewritingFixtureAsync(
        FakeRewritingClient primary,
        FakeRewritingClient fallback,
        IReadOnlyDictionary<string, string?>? overrides = null)
    {
        var fake = new FakeRewritingProvider(primary, fallback);
        var fixture = await OperationFixture.CreateAsync(
            ...
            services =>
            {
                ...
                services.AddSingleton<IRewritingClientProvider>(fake);
            });
        return (fixture, fake);
    }
    ...
    private static string SubmitBody(string operationId, string source, string? mode = "simple", string? sourceSelection = null)
    {
    ...
    private static void EnqueueRewriteSuccess(FakeRewritingClient client, string language, string text) =>
        EnqueuePair(client, $"{{\"status\":\"eligible\",\"language\":\"{language}\"}}", $"{{\"status\":\"result\",\"text\":\"{text}\"}}");

    private static void EnqueuePair(FakeRewritingClient client, string eligibilityJson, string transformationJson)
    {
    ...
    [TestMethod]
    public async Task RewriteSubmitExecutesSynchronouslyWithChargeAndSnapshot()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        EnqueueRewriteSuccess(primary, "en", "REWRITTEN-SIMPLE-TEXT");
        var (fixture, _) = await CreateRewritingFixtureAsync(primary, fallback);
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewrite@example.test");
            const string source = "Please rewrite this report simply.";
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");

            using var response = await fixture.PostRawAsync("/api/operations", SubmitBody(operationId, source, mode: "simple"), access);
            ...
            Assert.AreEqual("rewriting", body.GetProperty("family").GetString());
            Assert.AreEqual("succeeded", body.GetProperty("status").GetString());
            Assert.AreEqual("REWRITTEN-SIMPLE-TEXT", body.GetProperty("rewrittenText").GetString());
            ...
            Assert.AreEqual("rewriting", stored.Family);
    ...
    [TestMethod]
    public async Task RewriteWithoutConfiguredProviderRemainsPending()
    {
        await using var fixture = await OperationFixture.CreateAsync();
        await fixture.CreateAccountAsync("rewritepending@example.test", confirmed: true);
        var access = await fixture.SignInAsync("rewritepending@example.test");
        var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");

        using var response = await fixture.PostRawAsync(
            "/api/operations",
            SubmitBody(operationId, "Reservation probe text", mode: "casual"),
            access);
    ...
    [TestMethod]
    public async Task OmittedModeDefaultsToCorrectionOnlyAndFingerprintsIdentically()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        EnqueueRewriteSuccess(primary, "en", "CORRECTED-TEXT");
        var (fixture, _) = await CreateRewritingFixtureAsync(primary, fallback);
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "modedefault@example.test");
            const string source = "Please correct this report.";
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");

            using var first = await fixture.PostRawAsync(
                "/api/operations",
                SubmitBody(operationId, source, mode: null),
                access);
            Assert.AreEqual(HttpStatusCode.Created, first.StatusCode);
            var firstBody = await first.Content.ReadFromJsonAsync<JsonElement>();
            Assert.AreEqual("CORRECTED-TEXT", firstBody.GetProperty("rewrittenText").GetString());
            Assert.AreEqual(2, primary.CallCount);

            using var replay = await fixture.PostRawAsync(
                "/api/operations",
                SubmitBody(operationId, source, mode: "correctionOnly"),
                access);
            ...
            using var conflict = await fixture.PostRawAsync(
                "/api/operations",
                SubmitBody(operationId, source, mode: "simple"),
                access);
    ...
    [TestMethod]
    public async Task CorrectionOnlyUnchangedTextStillSucceedsWithFullCharge()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        const string source = "This report is already correct.";
        EnqueueRewriteSuccess(primary, "en", source);
        var (fixture, _) = await CreateRewritingFixtureAsync(primary, fallback);
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "unchanged@example.test");
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");
            ...
            Assert.AreEqual(HttpStatusCode.Created, response.StatusCode);
            var body = await response.Content.ReadFromJsonAsync<JsonElement>();
            Assert.AreEqual(source, body.GetProperty("rewrittenText").GetString());
            Assert.AreEqual(source.Length, body.GetProperty("characterCount").GetInt32());
    ...
    [TestMethod]
    public async Task RewriteValidationMatrixRejectsWithoutDispatchOrCharge()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        var (fixture, _) = await CreateRewritingFixtureAsync(primary, fallback);
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewritevalidation@example.test");
            ...
            var cases = new (string Name, string Body, HttpStatusCode Status, string Category)[]
            {
                ...
                ("unknown-field", $"{{\"operationId\":\"{OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()):D}\",\"family\":\"rewriting\",\"source\":\"Hello.\",\"modde\":\"simple\"}}", HttpStatusCode.BadRequest, "invalidRequest"),
                ("unknown-mode", SubmitBody(OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D"), "Hello.", mode: "flowery"), HttpStatusCode.UnprocessableEntity, "inputEligibility"),
                ("combined-modes", SubmitBody(OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D"), "Hello.", mode: "simple,casual"), HttpStatusCode.UnprocessableEntity, "inputEligibility"),
                ("invalid-source-selection", SubmitBody(OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D"), "Hello.", sourceSelection: "xx"), HttpStatusCode.UnprocessableEntity, "inputEligibility"),
                ...
                ("invalid-unicode", $"{{\"operationId\":\"{OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()):D}\",\"family\":\"rewriting\",\"source\":\"a\\ud800b\",\"mode\":\"simple\"}}", HttpStatusCode.BadRequest, "invalidRequest"),
            };
            ...
            var duplicateKey = $"{{\"operationId\":\"{OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()):D}\",\"operationId\":\"{OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()):D}\",\"family\":\"rewriting\",\"source\":\"Hello.\",\"mode\":\"simple\"}}";
    ...
    [TestMethod]
    public async Task RewriteEligibilityRejectionsEndWithoutTransformationOrCharge()
    {
        ...
        foreach (var (name, classification, hint, reason) in rejections)
        {
            ...
            await using (fixture)
            {
                var access = await CreateVerifiedAccountAsync(fixture, $"rewrite{name}@example.test");
                var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");
    ...
    [TestMethod]
    public async Task DuplicateRewriteReplaysMetadataWithoutNewChargeOrDispatch()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        EnqueueRewriteSuccess(primary, "en", "REWRITTEN-TEXT");
        var (fixture, _) = await CreateRewritingFixtureAsync(primary, fallback);
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewriteduplicate@example.test");
            const string source = "Charge exactly once for rewriting.";
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");
            var body = SubmitBody(operationId, source, mode: "business");
            ...
            using var conflict = await fixture.PostRawAsync(
                "/api/operations",
                SubmitBody(operationId, "A different rewriting payload.", mode: "business"),
                access);
            Assert.AreEqual(HttpStatusCode.Conflict, conflict.StatusCode);
            var conflictBody = await conflict.Content.ReadFromJsonAsync<JsonElement>();
            Assert.AreEqual("identityConflict", conflictBody.GetProperty("category").GetString());
    ...
    [TestMethod]
    public async Task TransientRewriteFailuresExhaustToProcessingFailure()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        ...
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewritetransient@example.test");
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");

            using var response = await fixture.PostRawAsync(
                "/api/operations",
                SubmitBody(operationId, "Please rewrite this report."),
                access);
    ...
    [TestMethod]
    public async Task InvalidRewriteOutputExhaustsToProcessingFailure()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        ...
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewriteinvalidoutput@example.test");
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");
    ...
    [TestMethod]
    public async Task RewriteRefusalsExhaustToProcessingFailure()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        ...
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewriterefusal@example.test");
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");
    ...
    [TestMethod]
    public async Task RewriteFallbackReceivesOriginalSourceAndMode()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        ...
        fallback.EnqueueResponse("{\"status\":\"result\",\"text\":\"FALLBACK-REWRITTEN-TEXT\"}");
        var (fixture, _) = await CreateRewritingFixtureAsync(primary, fallback);
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewritefallback@example.test");
            const string source = "Please rewrite this report simply.";
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");
            ...
            Assert.AreEqual(HttpStatusCode.Created, response.StatusCode);
            var body = await response.Content.ReadFromJsonAsync<JsonElement>();
            Assert.AreEqual("FALLBACK-REWRITTEN-TEXT", body.GetProperty("rewrittenText").GetString());
            Assert.AreEqual(2, primary.CallCount);
            Assert.AreEqual(1, fallback.CallCount);
            ...
            Assert.AreEqual("simple", request.RootElement.GetProperty("mode").GetString());
        }
    }

    [TestMethod]
    public async Task RewriteDeadlineExpiryFencesExecutionWithZeroCharge()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        ...
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewritedeadline@example.test");
            var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");
    ...
    [TestMethod]
    public async Task DeniedRewriteMonetaryAdmissionStopsDispatchWithSuspension()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        EnqueueRewriteSuccess(primary, "en", "REWRITTEN-TEXT");
        ...
        await fixture.CreateAccountAsync("rewritesuspended@example.test", confirmed: true);
        var access = await fixture.SignInAsync("rewritesuspended@example.test");
        var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");
    ...
    [TestMethod]
    public async Task ExhaustedUserAllowanceDeniesRewriteWithoutDispatch()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        EnqueueRewriteSuccess(primary, "en", "FIRST-RESULT");
        EnqueueRewriteSuccess(primary, "en", "SECOND-RESULT");
        var (fixture, _) = await CreateRewritingFixtureAsync(
            primary,
            fallback,
        ...
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewriteallowance@example.test");
    ...
    [TestMethod]
    public async Task SettledRewriteSurvivesRestartWithDispatchFreeReads()
    {
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        EnqueueRewriteSuccess(primary, "en", "REWRITTEN-TEXT");
        var (fixture, _) = await CreateRewritingFixtureAsync(primary, fallback);
        var access = await CreateVerifiedAccountAsync(fixture, "rewriterestart@example.test");
        const string source = "Restart durability rewriting probe.";
        var operationId = OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D");

        using (await fixture.PostRawAsync("/api/operations", SubmitBody(operationId, source, mode: "friendly"), access))
        {
        }
        ...
        {
            var restartedAccess = await restarted.SignInAsync("rewriterestart@example.test");
            using var status = await restarted.GetAsync($"/api/operations/{operationId}", restartedAccess);
            Assert.AreEqual(HttpStatusCode.OK, status.StatusCode);
            var statusBody = await status.Content.ReadFromJsonAsync<JsonElement>();
            Assert.AreEqual("succeeded", statusBody.GetProperty("status").GetString());
            Assert.AreEqual("rewriting", statusBody.GetProperty("family").GetString());
            Assert.IsFalse(statusBody.GetProperty("outputAvailable").GetBoolean());
            Assert.AreEqual(source.Length, statusBody.GetProperty("characterCount").GetInt32());
    ...
    [TestMethod]
    public async Task RewriteFailureCarriesNoSourceTextOrSecrets()
    {
        const string sourceSentinel = "SENTINEL-REWRITE-SOURCE-9f3c2a";
        const string resultSentinel = "SENTINEL-REWRITE-RESULT-7a1e4b";
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        primary.EnqueueThrow(new HttpRequestException("synthetic provider failure"));
        fallback.EnqueueThrow(new HttpRequestException("synthetic provider failure"));
        ...
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewriteprivacy@example.test");

            using var failed = await fixture.PostRawAsync(
                "/api/operations",
                SubmitBody(OperationIdentity.CreateForTime(fixture.Time.GetUtcNow()).ToString("D"), $"Rewrite {sourceSentinel} now."),
                access);
    ...
    [TestMethod]
    public async Task RewriteSuccessOmitsSourceTextOutsideRewrittenText()
    {
        const string sourceSentinel = "SENTINEL-REWRITE-SOURCE-55aa01";
        using var primary = new FakeRewritingClient();
        using var fallback = new FakeRewritingClient();
        EnqueueRewriteSuccess(primary, "en", "CLEAN-REWRITTEN-RESULT");
        var (fixture, _) = await CreateRewritingFixtureAsync(primary, fallback);
        await using (fixture)
        {
            var access = await CreateVerifiedAccountAsync(fixture, "rewritesuccessprivacy@example.test");
            ...
            Assert.AreEqual(HttpStatusCode.Created, response.StatusCode);
            var body = await response.Content.ReadFromJsonAsync<JsonElement>();
            Assert.AreEqual("CLEAN-REWRITTEN-RESULT", body.GetProperty("rewrittenText").GetString());
    ...
    [TestMethod]
    public void RewriteAndTranslationSuccessEnvelopesStaySymmetric()
    {
        var rewriting = typeof(RewritingSuccessResponse).GetProperties();
        var translation = typeof(TranslationSuccessResponse).GetProperties();
        CollectionAssert.AreEquivalent(
            rewriting.Select(property => property.Name).Where(name => name != nameof(RewritingSuccessResponse.RewrittenText)).ToArray(),
            translation.Select(property => property.Name).Where(name => name != nameof(TranslationSuccessResponse.TranslatedText)).ToArray());
        foreach (var rewritingProperty in rewriting.Where(property => property.Name != nameof(RewritingSuccessResponse.RewrittenText)))
        {
            var translationProperty = translation.Single(property => property.Name == rewritingProperty.Name);
            Assert.AreEqual(rewritingProperty.PropertyType, translationProperty.PropertyType, rewritingProperty.Name);
        }

        Assert.AreEqual(typeof(string), typeof(RewritingSuccessResponse).GetProperty(nameof(RewritingSuccessResponse.RewrittenText))!.PropertyType);
        Assert.AreEqual(typeof(string), typeof(TranslationSuccessResponse).GetProperty(nameof(TranslationSuccessResponse.TranslatedText))!.PropertyType);
        Assert.IsNotNull(
            typeof(RewritingSuccessResponse).GetProperty(nameof(RewritingSuccessResponse.RewrittenText))!
                .GetCustomAttribute<System.ComponentModel.DescriptionAttribute>(),
            "The generated contract gate requires a description on the rewritten text.");
        Assert.IsNotNull(
            typeof(TranslationSuccessResponse).GetProperty(nameof(TranslationSuccessResponse.TranslatedText))!
                .GetCustomAttribute<System.ComponentModel.DescriptionAttribute>(),
            "The restored translation component requires a description on the translated text.");
    }

    private sealed class FakeRewritingProvider(
        FakeRewritingClient primary,
        FakeRewritingClient fallback) : IRewritingClientProvider
    {
    ...
    private sealed class FakeRewritingClient : IChatClient, IDisposable
    {
        ...
        public IAsyncEnumerable<ChatResponseUpdate> GetStreamingResponseAsync(
            ...
            ChatOptions? options = null,
            CancellationToken cancellationToken = default) =>
            throw new NotSupportedException("The rewriting fake never streams.");
    }
}
