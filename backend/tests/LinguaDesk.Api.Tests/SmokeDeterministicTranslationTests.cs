...
[TestClass]
public sealed class SmokeDeterministicTranslationTests
{
    ...
    {
        var provider = new DeterministicSmokeTranslationProvider();
    ...
    {
        var client = DeterministicSmokeTranslationClient.Instance;
        var response = await client.GetResponseAsync(
            [UserPrompt("""{"source":"Hello, the meeting starts at 14:30. Please go.","sourceHint":null}""")],
    ...
    public async Task EligibilityRejectSentinelIsUnsupportedWithoutLanguage()
    {
        var client = DeterministicSmokeTranslationClient.Instance;
        var response = await client.GetResponseAsync(
            [UserPrompt("""{"source":"Hello M028-ELIGIBILITY-REJECT","sourceHint":null}""")],
            cancellationToken: TestContext.CancellationToken);
        var result = ParseResult(ResponseText(response));
    ...
    [TestMethod]
    public async Task TransformationCallReturnsFixedRomanianFixture()
    {
        var client = DeterministicSmokeTranslationClient.Instance;
        var response = await client.GetResponseAsync(
            [UserPrompt("""{"source":"Hello, the meeting starts at 14:30. Please go.","sourceLanguage":"en","targetLanguage":"ro"}""")],
            cancellationToken: TestContext.CancellationToken);
        var result = ParseResult(ResponseText(response));
        Assert.AreEqual("result", result.GetProperty("status").GetString());
        Assert.AreEqual(
            "Bună, întâlnirea începe la 14:30. Te rog să mergi.",
    ...
    public async Task ProcessingFailureSentinelThrowsTransiently()
    {
        ...
        await Assert.ThrowsExactlyAsync<HttpRequestException>(() => client.GetResponseAsync(
            [UserPrompt("""{"source":"M028-PROCESSING-FAILURE probe","sourceLanguage":"en","targetLanguage":"ro"}""")],
            cancellationToken: TestContext.CancellationToken));
    }
    ...
    public void RegistrationKeepsUnavailableProviderOutsideSmoke()
    {
        ...
        services.AddSmokeDeterministicTranslationProvider(new TestHostEnvironment("Production"));
        var registrations = services
            .Where(descriptor => descriptor.ServiceType == typeof(ITranslationClientProvider))
            .ToList();
        Assert.HasCount(1, registrations);
        Assert.AreEqual(typeof(UnavailableTranslationClientProvider), registrations[0].ImplementationType);
    }
    ...
    public void RegistrationReplacesProviderOnlyInSmoke()
    {
        var services = new ServiceCollection();
        services.AddScoped<ITranslationClientProvider, UnavailableTranslationClientProvider>();
        services.AddSmokeDeterministicTranslationProvider(new TestHostEnvironment("Smoke"));
        var registrations = services
            .Where(descriptor => descriptor.ServiceType == typeof(ITranslationClientProvider))
            .ToList();
        Assert.HasCount(1, registrations);
        Assert.AreEqual(typeof(DeterministicSmokeTranslationProvider), registrations[0].ImplementationType);
    }
