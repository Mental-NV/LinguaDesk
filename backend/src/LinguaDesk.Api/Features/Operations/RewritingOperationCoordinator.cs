...
public enum RewritingExecutionOutcome
{
    Succeeded,
    ProviderUnavailable,
...
public sealed record RewritingExecutionResult(
    RewritingExecutionOutcome Outcome,
...
/// <summary>
/// Executes one admitted rewriting operation synchronously: per-dispatch
/// monetary admission, M017 rewriting pipeline plus M018 explicit chain
...
/// zero character charge. A valid correction-only pass may leave correct text
/// unchanged; that is still a success charging the full submitted count.
/// </summary>
public sealed partial class RewritingOperationCoordinator(
...
{
    internal const string NoChargeEvidenceReference = "fake-rewriting-harness-v1";

    public async Task<RewritingExecutionResult> ExecuteAdmittedAsync(
        string accountId,
        string source,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!clients.TryGetClients(out var router, out var chain) || router is null || chain is null)
        {
            ...
            return new(
                RewritingExecutionOutcome.ProviderUnavailable,
                submission,
                admittedUsage,
                admittedTime,
                Dispatches: 0);
        ...
        try
        {
            ...
            policy.Validate();
            outcome = await ChainOrchestrator.ExecuteRewritingAsync(
                chain,
        ...
        catch (OperationCanceledException)
        {
            ...
            var interrupted = await recovery.InterruptAsync(
                accountId,
                OperationAdmissionService.FamilyRewriting,
                source,
                sourceSelection,
                target: null,
                effectiveMode,
            ...
            return new(
                RewritingExecutionOutcome.Cancelled,
        ...
        switch (outcome.Decision)
        {
            case ChainDecision.Succeeded when !string.IsNullOrWhiteSpace(outcome.Text):
                ...
                return new(
                    RewritingExecutionOutcome.Succeeded,
                    settled.Submission,
                    settled.Usage,
                    settled.ServerTime,
            ...
            case ChainDecision.Succeeded:
                return await FailAsync(
                    accountId, source, sourceSelection, effectiveMode, operationId, submission, gate,
                    RewritingExecutionOutcome.ProcessingFailed, eligibilityReason: null, cancellationToken).ConfigureAwait(false);
            case ChainDecision.Ineligible:
                return await FailAsync(
                    accountId, source, sourceSelection, effectiveMode, operationId, submission, gate,
                    RewritingExecutionOutcome.InputEligibilityRejected,
    ...
    private async Task<RewritingExecutionResult> FailAsync(
        string accountId,
        string source,
        string? sourceSelection,
        string? mode,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            RewritingExecutionOutcome.InputEligibilityRejected => nameof(RewritingExecutionOutcome.InputEligibilityRejected),
            RewritingExecutionOutcome.ProcessingFailed => nameof(RewritingExecutionOutcome.ProcessingFailed),
            RewritingExecutionOutcome.DeadlineExceeded => nameof(RewritingExecutionOutcome.DeadlineExceeded),
            RewritingExecutionOutcome.MonetarySuspended => nameof(RewritingExecutionOutcome.MonetarySuspended),
            _ => nameof(RewritingExecutionOutcome.ProcessingFailed),
        };
    ...
    {
        [LoggerMessage(EventId = 1, Level = LogLevel.Information, Message = "Rewriting execution unavailable: {ScalarCount} characters remain pending with no provider dispatch.")]
        public static partial void ProviderUnavailable(ILogger logger, int scalarCount);

        [LoggerMessage(EventId = 2, Level = LogLevel.Information, Message = "Rewriting succeeded: {ScalarCount} characters charged on {AdmissionDay} after {Dispatches} dispatches.")]
        public static partial void Succeeded(ILogger logger, int scalarCount, string admissionDay, int dispatches);

        [LoggerMessage(EventId = 3, Level = LogLevel.Information, Message = "Rewriting settled as failure: {ScalarCount} characters {Outcome} after {Dispatches} dispatches with zero charge.")]
        public static partial void SettledFailure(ILogger logger, int scalarCount, string outcome, int dispatches);

        [LoggerMessage(EventId = 4, Level = LogLevel.Information, Message = "Rewriting success arrived after the stored deadline and was fenced: {ScalarCount} characters with zero charge after {Dispatches} dispatches.")]
        public static partial void LateSuccessFenced(ILogger logger, int scalarCount, int dispatches);

        [LoggerMessage(EventId = 5, Level = LogLevel.Information, Message = "Rewriting execution cancelled: {ScalarCount} characters fenced after {Dispatches} dispatches.")]
        ...
        MonetaryAdmissionService monetary)
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var result = await monetary.AdmitAsync(
                $"{operationReference}:rewriting-attempt-{Dispatches}",
                operationReference,
                Dispatches,
            ...
            IChatClient inner) : IChatClient
        {
            ...
            public IAsyncEnumerable<ChatResponseUpdate> GetStreamingResponseAsync(
                ...
                ChatOptions? options = null,
                CancellationToken cancellationToken = default) =>
                throw new NotSupportedException("The rewriting execution path never streams.");
        }
    }

    private sealed class MonetaryDeniedException(MonetaryAdmissionOutcome outcome) : InvalidOperationException(
        $"Monetary admission denied rewriting dispatch: {outcome}.")
    {
    }
