...
**Change note — 2026-09-15:** After M036 completed, manual portal testing showed `run` serves Translate/Rewrite with no LLM behind them (unavailable providers → 202 pending → processing-failure message with a futile retry). Added M043 (live serving under a $0.05-per-operation cap with fail-fast startup, plus opt-in env-gated live e2e suites) with owning proposal at `docs/08-backlogs/M043/proposal.md`. Serving candidate (DeepSeek-V4.1-Flash), env-only credentials, cap, and fail-fast are owner-locked. No completed milestone or ID changed.
