...
**Document:** #7 · **Version:** 1.18 · **Status:** Current design; post-M025 dedicated-user UI/E2E delivery sequence explicit; implementation/evidence status is maintained in delivery/current.md and verification/coverage.md
**Updated:** 2026-09-11
...
Every UI-facing milestone must allocate published E2E cases using #6's predefined verified user. At least one suite case signs in through the visible login form; remaining cases may reuse only #6's real-auth Playwright state. Each UI feature is incomplete until an E2E case performs its user action and asserts its visible outcome. Backend/API success, a test database row or network-only browser assertion is supporting evidence, not the UI exit.
...
3. **After completed M025, deliver user-visible vertical slices:** select M026 only as the Translation API enabler, then complete M028 so a verified user can translate in the published UI before beginning the Rewriting API/UI pair M027 → M029. If M026 or M028 is genuinely blocked, [delivery status](delivery/current.md) records the blocker before another eligible milestone is selected. M026/M027 API evidence does not count as an end-user web outcome.
4. After both basic UI journeys exist, complete M030–M032 for competing events, visible recovery and workspace teardown. Integrate accessibility, privacy and failure checks with each feature; M038/M039 and G3 retain the full release browser/device/AT obligation.
5. Begin bounded corpus/evaluation and operational preparation as soon as inputs are available without displacing the immediate M026 → M028 → M027 → M029 product sequence unless a recorded dependency blocks it; do not leave qualification risk until the final release review.
...
This is the user-visible delivery track. After M001–M025, the intended sequence is **M026 → M028 → M027 → M029**: finish one complete Translation vertical slice before starting the corresponding Rewriting pair. M026 and M027 expose independently useful APIs, but only M028 and M029 establish that users can perform the operations in the UI. Section 4.5 of [verification plan #6](06-verification-plan.md#45-required-user-visible-transformation-evidence) is part of their Done criteria.
...
| M028 | Translate text in the web workspace | The predefined verified E2E user opens `/translate`, selects a valid source/target, enters text and activates **Translate** once through the published SPA → auth → API → accounting → deterministic-provider path. Dedicated-user E2E cases assert in the UI one complete editable/copyable result, authoritative usage, and preservation/no-charge behavior for invalid, oversized and controlled-failure cases; at least one published suite case has authenticated through the real login form. | M013, M026 | FR-003–011/023/024/028; UX-AC-021/022/024/030/033/036/037/065; V-001/V-002/V-003/V-005/V-010/V-012 |
| M027 | Rewrite through the authenticated API | The same client explicitly rewrites with the selected mode and the same durable limits, recovery and complete-result semantics. This enables but does not complete the web outcome. | M026, M017 | FR-012–018/024/035–037; V-005/V-007/V-009 |
| M029 | Rewrite text in the web workspace | The predefined verified E2E user opens `/rewrite`, enters text, keeps **Correction only** or chooses one supported mode, and activates **Rewrite** once through the published integrated path. Dedicated-user E2E cases assert in the UI one complete same-language editable/copyable result, authoritative usage, unchanged source and preservation behavior for invalid/oversized input, failure and protected result edits. | M028, M027 | FR-012–018/022–024/028; UX-AC-038–047/051/065; V-001/V-002/V-003/V-005/V-010/V-012 |
| M030 | Keep editing across competing events | Dedicated-user published E2E cases plus focused checks prove cross-page source/settings/result edits defeat stale callbacks while the UI preserves current text and successful discarded work still updates displayed usage. | M029 | FR-017/018/022/023/026; UX §6; V-002/V-005/V-010/V-012 |
| M031 | Recover visibly from unavailable or unknown outcomes | Dedicated-user published E2E cases assert that the user distinguishes definitive failure, unavailable usage and unknown outcome in the UI; explicit retry/status/refresh actions do not cause unintended language work. | M030, M024, M025 | FR-028/034/037; UX §6/9; V-002/V-003/V-005/V-012 |
| M032 | End and reset the workspace safely | Dedicated-user published E2E cases assert that reset, session teardown and real navigation/restoration clear private text and restore visible defaults while independently settled usage remains correct. | M031 | FR-038; NFR-004; UX §3; V-002/V-010/V-012/V-015 |
...
| M034 | Unblock local registration without email | Every newly created local account is durably verified without delivery, receives a generic sign-in-required acknowledgment and can proceed through explicit sign-in; duplicate registration cannot alter an existing account. Real verification/reset mail is deferred under DF-008. | M006, M011, M013 | FR-001/002; Q-006; V-003/V-004/V-009/V-012/V-015 |
| M043 | Serve live translation/rewriting in the portal with bounded spend | A verified user completes Translate and Rewrite in the portal through the serving candidate under a $0.05-per-operation cap; `run` without serving configuration fails fast instead of serving a dead portal; opt-in env-gated live suites prove the portal path end-to-end against real LLM calls without entering default gates. Owning proposal: `docs/research/m043-live-serving-proposal-2026-09-15.md` (the `docs/08-backlogs/M043/` package is created by the plan phase, not in advance). | M026, M027, M028, M029, M019; Q-001 | FR-003/011/012/024; Q-001; V-002/V-005/V-009 |
...
| M038 | Verify one desktop Translation/Rewriting journey group | One bounded desktop group exercises the working Translation and Rewriting UI journeys with the required native interaction evidence and reviewed baselines. | M014, M032 | NFR-005/007; verification §4; V-010/V-011 |
| M039 | Verify one narrow-screen Translation/Rewriting journey group | One bounded narrow-screen group exercises the working Translation and Rewriting UI journeys with reflow, control and accessibility evidence; actual-device/manual gaps are explicit. | M014, M032 | NFR-005; verification §4; V-010/V-011 |
...
| G3 — Supported web access | Full agreed current/previous actual-browser, device, keyboard and AT evidence for the working Translation and Rewriting journeys under #2/#6 | Functional journeys from M028/M029, M038/M039 plus successors and qualified manual review; access to actual versions/devices/reviewers. Bundled engines alone are insufficient |
| G4 — Operational and specification readiness | Actual monetary cap and serving configuration; explicit DF-008 automatic-verification risk disclosure; retention/reconciliation/account lifecycle decisions; published migration/restore/privacy evidence and disposition of applicable launch questions | M034/M040/M041 plus successors; Q-001/Q-004/Q-006/Q-008/Q-010. Deferred mail evidence is excluded until DF-008 is reactivated; PRD owners review proposals before any new safeguard or compatibility obligation is adopted |

Q-003/Q-006's selected wire/auth-policy/bootstrap details are resolved before their affected handlers/clients. Real account-email delivery is the explicit DF-008 exception and does not block the current MVP. M006 explicitly stages durable account/key records without a deletion or backup-retention claim; Q-004 still blocks those lifecycle behaviors and launch evidence when introduced. Offline scaffolding or scripted AI work does not need live credentials or a production cap; every live run needs its own finite admitted budget. Provider-managed caching does not waive application privacy, successful-character accounting or cost verification, and provider retention/no-training certification is not a release checkpoint.
...
**Change note — 2026-09-11:** After M001–M025 completed, the next-delivery policy was clarified as M026 → M028 → M027 → M029. Existing IDs and dependencies are preserved; M028/M029 exits now state the end-user UI outcomes and their minimum integrated evidence explicitly. The E2E contract subsequently fixed the dedicated verified test identity, required at least one real login-form path, constrained the reusable authentication fixture and required browser-visible assertions for every UI-facing feature. No completed milestone or deferred scope changed.

**Change note — 2026-09-15:** After M036 completed, manual portal testing showed `run` serves Translate/Rewrite with no LLM behind them (unavailable providers → 202 pending → processing-failure message with a futile retry). Added M043 (live serving under a $0.05-per-operation cap with fail-fast startup, plus opt-in env-gated live e2e suites) with owning proposal at `docs/research/m043-live-serving-proposal-2026-09-15.md`. Serving candidate (DeepSeek-V4.1-Flash), env-only credentials, cap, and fail-fast are owner-locked. No completed milestone or ID changed. (An early `docs/08-backlogs/M043/` package with only backlog/proposal was removed: `context.py audit` requires spec/plan/tasks in every backlog dir, and that package is created by the plan phase.)
