...
Use V-016's fresh migration/model drift portions and V-005's file-backed transaction/persistence foundation. Run relevant V-009/V-012 host/published regressions. The shared strategy remains [#6](../../06-verification-plan.md); this package defines only its selected assertions. Authentication, account isolation, language quality, monetary policy and full accessibility/device review are not applicable because this increment introduces none of those capabilities.
