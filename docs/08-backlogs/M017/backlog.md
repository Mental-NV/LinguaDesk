...
Human steps: owner credential already staged via M019 (no new access action); executor runs the single bounded live slice; human reviews the sanitized report at handoff (mode-behavior and Simplified-output spot review). A missing credential blocks AC-004's live portion only.
