...
**Most recently completed milestone: [M025 — Report ordered usage across period changes](../08-backlogs/M025/backlog.md).** State: **Done**. [Package 025](../08-backlogs/M025/spec.md) AC-001–008 passed with deterministic usage/availability reporting + file-backed SQLite/HTTP evidence and zero provider dispatches: a verified-account `GET /api/usage` read and every operation submit/duplicate/status response carry an authoritative current-day snapshot with a categorical availability signal (`available`/`user-exhausted`/`service-exhausted`/`monetary-suspended`, storage failure mapping to 503 `availability` without values) computed from one consistent user/global/monetary/revision read; cross-midnight success settles on the stored admission day with a fresh new-day snapshot and stale-charge disclosure; cross-month unresolved exposure denies new-month admission with suspended availability until authoritative settle-once attribution in the original month; snapshots order by UTC day then durable revision across restarts and JSON round-trips with no cross-account comparison; duplicate/status reads reuse the original period with no ledger change; interrupted/failed reads across midnight report zero charge; ledgers/revision/availability survive restart with side-effect-free reads; and the reviewed `UsageAvailability`/`GET /api/usage` delta ships with zero OpenAPI/TypeScript drift and no source text, global counts or minor-unit values in problems, logs or TEXT columns. Its [completion record](../08-backlogs/M025/tasks.md#completion-record) owns the exact evidence. Real dispatch (M026/M027) does not exist yet; this slice is not serving.
