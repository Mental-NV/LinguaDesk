...
**Most recently completed milestone: [M028 — Translate text in the web workspace](../08-backlogs/M028/backlog.md).** State: **Done**. [Package 028](../08-backlogs/M028/spec.md) AC-001–008 passed with the published deterministic-provider journey + file-backed SQLite/browser evidence: the predefined verified E2E user opens `/translate`, selects a valid source/target, waits with zero transformation requests, activates **Translate** once, receives one complete editable/copyable deterministic result and sees authoritative usage 7,546 (seed 7,500 + 46); typing/pasting/language changes/elapsed time/IME composition never submit and duplicate activation is suppressed; empty/oversize (excess 312)/same-language inputs are blocked with no charge; manual result edits and exact-value copies start no operation while pending responses are fenced and later submissions may replace edits; the controlled 503 failure preserves work with zero usage change and retries with a new operation key; at least one published case signs in through the real login form while others reuse the documented real-auth cookie fixture; the M026 wire shape ships with zero OpenAPI/TypeScript drift and no source/result text, credentials, global counts, monetary values or provider internals in problems, logs, traces or TEXT columns. Its [completion record](../08-backlogs/M028/tasks.md#completion-record) owns the exact evidence. Only the Smoke deterministic provider dispatches; rewriting dispatch (M027), the rewriting journey (M029) and live serving do not exist yet; this slice is not serving.
...
**Next recommended delivery train:** M028 → M027 → M029. M028 established the first end-user language outcome; deliver M027 next as the bounded rewriting API enabler, then M029 for the rewriting journey under the same dedicated-user contract, unless a recorded blocker requires another eligible choice. At least one published E2E case must sign in through the visible login form; other cases may use only #6's real-auth fixture. M026/M027 alone remain independent-client outcomes and do not satisfy either web journey.
...
All roadmap milestones other than completed M001–M026 and M028 are Candidate with no selected package and evidence pending. The M028 → M027 → M029 recommendation does not select or create a package; the next planning pass must lock M027 under #0. Full product and release evidence remains pending in #6 coverage.
