...
**Most recently completed milestone: [M030 — Keep editing across competing events](../08-backlogs/M030/backlog.md).** State: **Done**. [Package 030](../08-backlogs/M030/spec.md) AC-001–007 passed with dedicated-user published E2E cases plus focused checks: per-feature workspace state lives above the router, so mode-link and Back/Forward navigation preserves each page's text/settings/result with zero transformation operations and destination `h1` focus; a hidden page's pending operation settles into its owning entry by captured revision without touching the visible page while its successful charge reconciles authoritative usage on return; in-page source/settings/result-edit guards, stale-charge disclosure, manual-edit protection, no auto-submit and per-page duplicate suppression hold on both pages; at least one case signs in through the real login form and the rest reuse the documented real-auth fixture; no source/result text, credentials, global counts, monetary values or provider internals in problems, logs, traces or TEXT columns; and the M026/M027 wire shapes are reused with zero OpenAPI/TypeScript drift. Its [completion record](../08-backlogs/M030/tasks.md#completion-record) owns the exact evidence. Only deterministic fake providers dispatch; live serving/quality/performance do not exist yet; this slice is not serving.
...
**Next recommended delivery train:** M030 → M031. M028/M029 delivered both web workspaces under the dedicated-user contract and M030 hardened them for competing events; deliver M031 next for visible recovery from unavailable or unknown outcomes, unless a recorded blocker requires another eligible choice. At least one published E2E case must sign in through the visible login form; other cases may use only #6's real-auth fixture.
...
All roadmap milestones other than completed M001–M030 are Candidate with no selected package and evidence pending. Full product and release evidence remains pending in #6 coverage.
