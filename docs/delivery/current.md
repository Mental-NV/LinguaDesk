...
**Most recently completed milestone: [M027 — Rewrite through the authenticated API](../08-backlogs/M027/backlog.md).** State: **Done**. [Package 027](../08-backlogs/M027/spec.md) AC-001–008 passed with deterministic fake-provider rewriting through the real host/accounting pipeline + file-backed SQLite/HTTP evidence: a verified Bearer [REDACTED] rewrite submit with exactly one mode executes synchronously behind the stored absolute deadline and returns `201` complete validated same-language text with one admission-day charge and a fresh current-day snapshot, or a classified `422`/`429`/`503`/`504` failure with zero charge; omitted mode defaults to `correctionOnly` and fingerprints identically; a valid correction-only pass may leave correct text unchanged with the full charge; local validation (including unknown/combined modes and unpaired surrogates) and eligibility rejections dispatch nothing; duplicates replay succeeded/output-unavailable metadata with no new charge or dispatch and changed payloads (including mode changes) conflict; transient/invalid/refusal faults exhaust the bounded chain (fallback receives the original source and mode) and deadline expiry fences late success, each with zero charge and released monetary exposure; per-dispatch monetary admission denies over-cap dispatch with `monetarySuspension`; exhausted allowances deny with `429` reset guidance; settled rewriting state survives restart with dispatch-free reads; and the reviewed `RewritingSuccessResponse` (`201` `anyOf` both envelopes, translation component byte-identical) delta ships with zero OpenAPI/TypeScript drift and no source/result text, global counts, monetary values, credentials or provider internals in problems, logs, traces or TEXT columns. Its [completion record](../08-backlogs/M027/tasks.md#completion-record) owns the exact evidence. Only deterministic fake providers dispatch; the rewriting journey (M029) and live serving do not exist yet; this slice is not serving.
...
**Next recommended delivery train:** M027 → M029. M027 delivered the bounded rewriting API enabler; deliver M029 next for the rewriting journey under the same dedicated-user contract, unless a recorded blocker requires another eligible choice. At least one published E2E case must sign in through the visible login form; other cases may use only #6's real-auth fixture. M026/M027 alone remain independent-client outcomes and do not satisfy either web journey.
...
All roadmap milestones other than completed M001–M028 are Candidate with no selected package and evidence pending. The M027 → M029 recommendation does not select or create a package; the next planning pass must lock M029 under #0. Full product and release evidence remains pending in #6 coverage.
