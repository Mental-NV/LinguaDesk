...
| FR-003 | UX §3; API §2 | V-003, V-009, V-012 | Candidate allocation: M026/M028 Translation and M027/M029 Rewriting; Unselected → Pending |
...
| FR-011 | UX §6–7; AI §4; API §6 | V-002, V-003, V-007, V-010, V-012 | [M016 AC-002/004](../08-backlogs/M016/spec.md#acceptance) complete validated plain-text result portion (single transformation dispatch, terminal recorded failure, no auto-submit/fallback in this slice) → Passed (development evidence); candidate M028 owns explicit UI activation/result handling; Unselected → Pending |
...
| API-AC-006 | V-005, V-009, V-015; M022 AC-007 passed the succeeded/failed terminal-read portion (original charge metadata with output unavailable, fresh usage, unknown stays 404 without asserting zero charge); M024 AC-001/AC-004 passed the interrupted/success-wins portion (restart-reconciled interrupted metadata with zero charge and fresh usage, aborted wait committing success with output unavailable and the original charge); M025 AC-006 passed the cross-midnight terminal-read portion (interrupted/failed status across midnight reports zero charge with the original admission day and a fresh current-day snapshot) |
...
| API-AC-008 | V-005, V-014; M024 AC-004 passed the disconnect/abort portion (aborted wait still commits success within the original deadline with succeeded/output-unavailable status; late output never revives terminal interrupted or failed records; status reads never dispatch work) |
| API-AC-009 | V-005, V-002, V-003; M025 AC-002 passed the cross-midnight portion (post-midnight success settles on the stored admission day with the new day untouched, status returns the admission day with a fresh new-day snapshot and stale-charge disclosure) |
