...
Keep fixed time `2026-09-07T17:40:00Z`, UTC display assertions, controlled deferred responses and fake timers only for deadlines/cooldowns/notices. There is **no debounce timer**. Count logical transformations separately from auth, usage, eligibility and status reads. DOM/component fixtures may use verified local `writer@example.test`; published E2E cases use the single predefined verified account `m013-verified@example.test` / `Maple!River2026`, seeded through ASP.NET Identity into the run-owned migrated SQLite database with user usage 7,500 and available global/budget capacity. Seed prior results explicitly and exclude seed operations from observed counts.

The published harness creates the account before serving browser traffic through the existing `Smoke`-only offline seed mode; no seed endpoint exists and non-Smoke startup must reject the mode. One E2E case must visit `/login`, fill the predefined credentials, activate **Sign in**, and assert the protected route plus visible authenticated content. A Playwright setup/helper may authenticate the same account for subsequent cases by obtaining antiforgery state and posting to the real published sign-in API, then reusing the returned same-origin cookie storage state. It may not forge cookies, mutate auth/session rows, disable middleware, intercept account or feature APIs, or substitute a test host. Keep one clean run-owned account/database baseline per suite or isolate stateful cases so usage and operation history cannot leak an undeclared precondition.

Every UI-facing feature selected by a milestone has published E2E coverage using that account. Exercise controls through semantic role/name locators and assert the visible browser result, status, validation, usage, focus, clipboard or cleared state relevant to the feature. Network request counts and database assertions may supplement but never replace the UI assertion. The selected package maps each UI-facing feature to one or more E2E cases and allocates its lower-level acceptance assertions to the appropriate checks; combinatorial policy cases do not require duplicate browser scripts when the visible feature outcome is already covered.
