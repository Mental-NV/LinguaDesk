...
| Q-006 | **Partially resolved:** [API design](../05-api-design.md) selects auth modes/lifecycle, identity and cancellation/recovery/error semantics. M034 selects automatic verification for newly created accounts as a temporary substitute; real confirmation/reset delivery details are deferred under DF-008. Schema version mechanics remain and P-006 compatibility guarantees are still proposed | Selected API/auth slices; DF-008 reactivation owns mail transport/authentication/live evidence before real delivery returns |
