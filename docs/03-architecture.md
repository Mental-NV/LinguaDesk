...
**Document:** #3 · **Version:** 1.15 · **Status:** Current design; implementation/evidence status is maintained in delivery/current.md and verification/coverage.md
**Updated:** 2026-09-11
...
PRD D-17–D-19 and Section 3 define current scope and later routing intent. P-001–P-004 retain their **newly amended** dispositions; P-005/NFR-008 and P-006 remain proposed. This document does not claim to close Q-001/Q-004/Q-008 in full. Active UX IDs remain behavioral contracts, not a mandatory browser-test count; Deferred/Retired IDs require no MVP implementation/evidence.
...
**Verification persistence boundary:** [Package 007](08-backlogs/M007/spec.md) retains anonymous body-based confirmation and resend operations. A dedicated named Data Protection email-confirmation provider has a 24-hour lifetime without changing the default password-reset provider. Under M034's temporary DF-008 substitute, registration creates new accounts with `EmailConfirmed = true` and performs no delivery or cooldown write. Resend for any pre-existing unverified account still uses its 60-second UTC attempt marker in `AspNetUserTokens`, serialized by the selected single-process account gate and recorded before the dormant unavailable sender boundary. No migration, queue or outbox is added. DF-008 owns restoration of registration delivery and real confirmation/reset mail; M009–M010 retain bearer credentials and password-reset/stamp behavior.

**Browser-session boundary:** [Package 008](08-backlogs/M008/spec.md) adds anonymous antiforgery bootstrap, strict local sign-in, current-session read and idempotent caller-cookie sign-out. The real Identity application cookie is `__Host-LinguaDesk.Session` (Secure, HttpOnly, SameSite=Lax, Path=/), nonpersistent, fixed at eight hours with no sliding renewal; every accepted cookie request reloads the account and compares its current security stamp. `__Host-LinguaDesk.Antiforgery` is Secure, HttpOnly, SameSite=Strict and paired with `X-LinguaDesk-Antiforgery` on sign-in/sign-out. A Bearer header selects a non-cookie path and cannot fall back while M009 remains absent. The slice adds no migration, account record, bearer/reset route, browser UX or account-wide revocation promise; M009, M010 and M013 retain those outcomes.
...
Bind validated typed options from non-secret configuration and explicitly named environment variables. Provider/email credentials stay exclusively on the backend; #4 owns the evaluation credential name and profile mapping. A provider credential may be shared by the evaluator and all LinguaDesk operations using that provider, so monetary admission and credential-scope failures must not assume evaluation, family or route isolation. Pin supported dependencies; never commit secrets. Fail startup on an invalid operation-family chain, missing required credentials, unwritable durable storage, or missing cost bounds for paid serving. Each current MVP chain requires one primary and permits at most one fallback; reject extra active candidates instead of silently activating DF-004. An explicit local/test configuration supplies fakes without requiring provider credentials.

Translation uses the same configured chain for all 12 directions; Rewriting uses its configured chain for all four languages and nine single-dropdown modes in the current MVP. Each configured candidate must qualify for every route/mode it serves. Preserve the PRD default-model preference subject to quality/performance/cost eligibility. Low-cost providers/models and provider-managed caching are allowed; no provider retention/no-training certification is required. D-19 confirms later route-specific provider selection and bounded multi-candidate fallbacks, but those remain deferred as DF-004. The candidate registry and evaluator can test multiple profiles now without implementing a priority rule engine or activating them for serving.
...
Admission enforces `known spend + unresolved exposure + new attempt upper bound <= configured cap` atomically across requests. The estimate covers all billable input, bounded output/reasoning tokens, and provider-specific charges for the two active operations. When evaluation and serving share a provider credential, their admitted LinguaDesk attempts also share the applicable exposure ledger or run only while the other path is proven quiescent; an isolated runner budget cannot claim to bound concurrent serving or unrelated account use. Any paid eligibility/detection attempt also needs cost admission even when no character charge results. Alternative context is deferred with DF-001; it adds no MVP accounting branch. An unknown price or unbounded billable attempt makes that configuration ineligible for paid dispatch. Reserve at the highest applicable tariff and cache-miss assumption until authoritative billing evidence supports less; release unused exposure only on authoritative evidence, because a timeout can still cost money. Provider-side hard budgets, when available, are an additional backstop.
