...
| [Use DeepL Write](https://support.deepl.com/hc/en-us/articles/11673757647388-Use-DeepL-Write) | Official documentation | Paired source/improved text, automatic improvement on web, green change marks, Show changes, alternatives, and clean copy behavior. |
...
| [Reset your password](https://support.deepl.com/hc/en-us/articles/360020693260-Reset-your-password) | Official documentation | Email-request journey and non-enumerating confirmation when an account may exist. |
...
The authenticated header contains, in order: LinguaDesk home link, `Translation`, `Rewriting`, flexible space, usage disclosure button, and account-menu button. On viewports below 768 px the two mode links remain visible as a two-item segmented row below the header; usage and account remain icon/text buttons in the header. There is no hamburger-only path to a core mode.
...
Settings and text live only in runtime memory. They must not be placed in URL parameters, browser history state payloads, local/session storage, IndexedDB, service-worker caches, autofill values, or analytics. Architecture defines teardown mechanics and may choose a stricter boundary, but may not visibly restore content after the boundary above.
...
Disabled text uses `#7C8794` on `#EEF1F4`; disabled state also removes pointer affordance and is exposed programmatically. Placeholder text is `#6D7885`. No state is communicated by color alone.
...
| `768–1199 px` compact desktop/tablet | 24 px gutters. Two equal editor columns. Rewriting tools open as a 360 px right-side sheet; trigger remains in result header. |
...
```text
┌ LinguaDesk ─ Translation ─ Rewriting ───────── Usage ▾  Account ▾ ┐
...
- Language and style labels wrap to two lines only in menus; closed controls truncate with an ellipsis after one line and expose the full value as accessible name and on focus/hover tooltip. The four supported language names fit at 320 px without abbreviation.
...
Tooltips appear after 500 ms hover or immediately on keyboard focus, never for disabled form fields whose helper is already visible, close on Escape/pointer exit/blur, and do not receive focus. Menus and nonmodal popovers close on Escape or outside pointer activation and return focus to their trigger. Modal dialogs trap focus, start on the heading or safest action (`Cancel`), close on Escape unless an irreversible submission is already underway, and return focus to their trigger.

### 5.2 Inventory

| Component | Purpose, content, variants, and accessible contract |
| --- | --- |
| `AppHeader` | Landmark `banner`; home link accessible name `LinguaDesk home`; contains `ModeNav`, `UsageDisclosure`, and `AccountMenu`. Never overlays focused content. |
...
| `LanguageSelect` | Combobox button. Source variant name `Source language`; target variant `Target language`; writing variant `Writing language`. Listbox supports arrows, Home/End, typeahead, Enter, Escape. Selected option has a checkmark and `aria-selected`. Source includes `Detect automatically`, English, Russian, Romanian, Chinese. Target omits the resolved source when known but retains it disabled with reason text to make same-language state understandable. |
...
| `ChangedText` | Revised spans use change background plus 2 px underline. They are not separately tabbable. The result's accessible text is clean text, not annotations. |
| `SentenceActions` | Toolbar button `Sentence actions`. Pointer activation on a result sentence opens the same actions directly. Keyboard users place the result caret in a sentence then press `Alt+ArrowDown`, or open the button and choose a sentence by accessible excerpt. The chosen sentence gets a 2 px outline and is exposed in the sheet heading. |
...
| `AlternativesPanel` | Heading `Alternatives for: <sentence excerpt>`; loading skeletons plus `Generating alternatives…`; 2–4 option buttons named `Use alternative: <full option>`; current text shown first as nonselectable `Current sentence`. Escape/close returns focus. |
...
Wide workspace order is: skip link, home, Translation, Rewriting, Usage, Account, source language, target language when present, source textbox, Clear, result textbox when editable, Sentence actions for rewriting, Show changes shortcut when rewriting, Copy, then tools rail controls in Correction only → Styles → Show changes order. Compact layouts keep the same logical sequence even when controls move visually. Popovers/sheets insert their contents after the trigger until closed. No generated sentence or changed word adds dozens of tab stops.
...
| --- | --- | --- | --- | --- |
| Empty | Editable, counter 0 | Placeholder `Your result will appear here.` | Language/settings enabled; Copy/Sentence actions disabled | Source on initial page entry |
...
| Offline | Editable | Previous result kept | No request; `Try again when online` informational state | Focus stays |
...
| UX-MSG-009 | `Only the first 5,000 characters will be translated and charged. {omitted} characters will not be sent.` | Translation source warning once canonical count exceeds limit; nondismissible while true. |
...
| UX-MSG-020 | `This update is no longer current and was not applied.` | Visual-only activity line when a response is discarded without charge update; clears on next current event. |
...
`{reset}`, `{time}`, `{count}`, `{language}`, `{omitted}`, and `{excess}` are populated only from authoritative operation/usage responses except provisional input count during typing. Missing reset time renders `when the daily allowance resets` rather than inventing a time. Error categories and fields remain document #5's responsibility.
...
On success, UX-MSG-010 remains above the result. `Copy result` copies only the returned prefix translation. Editing the source suffix still restarts the debounce because the canonical first 5,000 boundary may change under document #5's counting rules; the client must not assume suffix-only edits are free. A fixture can return `submittedCharacters: 5000` and `omittedCharacters: N`; those values govern display and charging disclosure.
...
| --- | --- | --- | --- | --- | --- |
| Empty or whitespace-only | Input remains ineligible | Empty | No error; target selectable; Copy disabled; source focus/caret unchanged | Cancel debounce; no request | UX-AC-021, 026 |
...
Grammar/spelling correction is always active. New-workspace defaults are Correction only off, Styles `None set`, and Show changes on. Style menu values and order are:
...
Pointer activation of a result sentence selects it and opens `SentenceMenu`; it does not change the source, source selection, or result text. Keyboard activation uses the caret sentence. If no caret is in the result, `Sentence actions` opens a listbox labelled `Choose a sentence`, each option named by its 40-character excerpt and ordinal. Focus moves to `Compare with source` when available, otherwise `Generate alternatives`.
...
| Edit undone exactly to the pre-edit sentence text/boundaries within the local undo stack | Metadata may be restored only if the preserved opaque revision still proves an exact match | Other metadata unchanged | Restored marks/comparison/cache may reappear; otherwise remains invalidated |
...
### 8.4 Full-rewrite state transitions
...
| Any pending | Source cleared/navigation/session ends | Empty/hidden/teardown per event | Menus close; focus per navigation rule | Pending invalidated; session end clears all | UX-AC-032, 005, 085 |

### 8.5 Alternatives state transitions

| Current state / preconditions | Trigger and guard | Next state and visible content | Focus | Logical effect | Scenarios |
| --- | --- | --- | --- | --- | --- |
| Current result, sentence selected, no cache | `Generate alternatives` | Loading; 3 skeleton rows and `Generating alternatives…` | Panel heading then remains on action for pointer; keyboard focus first skeleton container | Submit one sentence operation with current context/settings | UX-AC-052 |
...
| Loading current | Success with 2–4 options | Ready list; usage updates | Keyboard focus first option; pointer focus remains trigger | Cache one option set for sentence revision; charge once | UX-AC-052 |
...
| Loading current | Failure | Error UX-MSG-025; source/result unchanged | Focus `Try again` only if user invoked by keyboard; otherwise trigger | No cache/charge | UX-AC-056 |
...
| Selected/cache exists | Reopen sentence | Ready cached options; label `Show alternatives` | First selected/current option | No request/charge | UX-AC-054 |
...
- `Used: 7,500 characters` and `Remaining: 12,500 characters` (fixture example only; the 20,000 configured allowance comes from FR-024/server capability data).
...
- A network interruption after submission is `Processing interrupted. Checking the operation status…` only when the API contract can reconcile the operation. Until resolved, do not resubmit automatically or claim no charge. If the contract cannot reconcile, document #5 must provide the final user-visible category before implementation readiness.
...
- Under reduced motion, spinners may rotate only if essential to identify progress; otherwise use a static progress glyph. Transitions become 0 ms, skeleton shimmer is disabled, and no content flashes more than three times per second.
...
All acceptance IDs refer to Section 12. Shared behavior is normative in Sections 3–10 rather than duplicated per story.

| Story | User story | PRD links | Preconditions, entry point, and value | Screens/components/rules | Acceptance scenarios |
| --- | --- | --- | --- | --- | --- |
| UX-US-001 | As a verified user, I want to move between Translation and Rewriting without losing active work, so that I can use both tools in one workspace. | FR-003, FR-038, NFR-004 | Verified fixture; enter `/translate`, direct route, or mode link; continuity without saved history | Routes, ModeNav, Sections 3.2–3.4 | UX-AC-001–005 |
| UX-US-002 | As a new local user, I want to register and verify my email, so that I can use protected language operations. | FR-001, FR-002, FR-037 | Signed out; `/register`; controlled registration/verification outcomes | AuthForm, `/verify-email`, Section 9.3 | UX-AC-006–010 |
| UX-US-003 | As a user, I want to sign in with Google or local credentials and recover from auth errors, so that I can access my workspace securely. | FR-001, FR-002, FR-037 | Signed out or expired; protected route/login entry; deterministic auth callback | AuthForm, login banner, Sections 3.5/9.3 | UX-AC-011–016 |
| UX-US-004 | As a local user who forgot my password, I want to request and complete a reset, so that I can regain access without account disclosure. | FR-002, FR-037 | Signed out; `/forgot-password` or valid/invalid link | AuthForm, messages 031/033/036 | UX-AC-017–020 |
| UX-US-005 | As a language user, I want automatic detection or a manual supported source and an explicit translation target, so that the requested operation is unambiguous. | FR-004–FR-006, FR-009, FR-012 | Verified; either workspace page | LanguageSelect, Sections 7.1/8.1 | UX-AC-021, 027–029, 038, 088–091 |
...
| UX-US-007 | As a translation user, I want invalid, mixed, same-language, and oversized input handled clearly, so that I know what is processed and charged. | FR-004, FR-005, FR-007, FR-024, P-003 | Verified; `/translate`; controlled validation/count fixtures | Alerts, counter, boundary marker, Section 7.2 | UX-AC-026–032 |
...
| UX-US-009 | As a rewriting user, I want mandatory correction with one optional style or tone and a clear Correction-only override, so that the transformation is predictable. | FR-012–FR-016, FR-038 | New/active workspace; `/rewrite` | CorrectionOnly, StyleSelect, Section 8.1 | UX-AC-038–043 |
| UX-US-010 | As a rewriting user, I want automatic complete rewrites that preserve my source and apply only current responses, so that typing remains safe during processing. | FR-016–FR-018, FR-022, NFR-003 | Verified/available; `/rewrite`; eligible input | EditorPanel, full-rewrite transitions | UX-AC-044–047 |
| UX-US-011 | As a rewriting user, I want result-only change marks and original/revised sentence comparison, so that I can review corrections and copy clean text. | FR-019, FR-023, NFR-005 | Current rewrite result | ShowChanges, ComparisonPanel, Copy | UX-AC-048–051 |
| UX-US-012 | As a rewriting user, I want to generate, choose, and reuse sentence alternatives, so that I can refine one sentence without redoing the whole result. | FR-020, FR-021, FR-025, FR-035 | Current result with sentence metadata | SentenceMenu, AlternativesPanel, Section 8.5 | UX-AC-052–057 |
| UX-US-013 | As a rewriting user, I want manual edits to invalidate only affected sentence data and defeat earlier responses, so that my edits and unrelated choices remain intact. | FR-018, FR-022, FR-023, P-002, Q-002 | Current/pending rewrite result | Correspondence table, edit revision, messages 022–024 | UX-AC-058–064 |
| UX-US-014 | As a user, I want authoritative usage and clear allowance/budget states, so that I understand availability and successful charges. | FR-024–FR-028, NFR-006 | Verified; any protected page; usage fixtures | UsageDisclosure, counters, Section 9 | UX-AC-065–071 |
| UX-US-015 | As a user, I want failures, offline periods, and interrupted work to preserve text and offer safe recovery, so that transient problems do not destroy my work or cause blind retries. | FR-034, FR-037, NFR-002, NFR-003 | Any operation state; controlled failures | InlineAlert, retry, precedence | UX-AC-072–076 |
| UX-US-016 | As a keyboard, touch, zoom, or assistive-technology user, I want the full interface to remain operable and understandable, so that I can complete core journeys independently. | NFR-005, NFR-007, RG-006 | All routes; representative viewports/browsers | Sections 4, 5, 10 | UX-AC-077–083 |
...
| `T-OK-A` | Detect English; translate to Romanian: `Bună, întâlnirea începe la 14:30.`; submitted 43; usage becomes 7,543. |
| `T-OK-B` | Newer source returns `Raportul este gata.`; submitted 20; usage value supplied by fixture. |
| `T-UNCERTAIN` | Detection uncertain; no provider operation. |
| `T-MIXED` | Substantially mixed/unsupported validation; no provider operation. |
| `T-LONG` | Canonical total 5,312; submitted prefix 5,000; omitted 312; success usage +5,000. Fixture marks exact boundary. |
| `W-OK` | Two-sentence result with opaque sentence fixtures, word-level additions/deletions, and clean text. |
| `ALT-OK` | Exactly 3 alternatives for selected sentence; server returns sentence charge 42 and new usage. |
...
| UX-AC-006 / US-002 | `AUTH-OUT`, `/register`, valid local fields and password-policy fixture | Activate `Create account`, registration succeeds | One registration request; URL `/verify-email`; UX-MSG-034 and masked/safe email context visible; no protected workspace. |
...
| UX-AC-013 / US-003 | `/login`, valid local credentials | Submit; auth succeeds with original route `/translate` | One login request; replace-navigation `/translate`; source focused; no password remains in DOM/storage/history. |
...
| UX-AC-016 / US-003 | Verified workspace with text | Open account menu and activate `Sign out` | One sign-out action; immediate teardown; `/login`; Back does not expose text. |
| UX-AC-017 / US-004 | `/forgot-password`, syntactically valid unknown email | Submit success fixture | One request; UX-MSG-031 exactly; same result as known-email fixture; `Back to sign in` focused or first in reading order. |
...
| ID / story | Given | When | Then |
| --- | --- | --- | --- |
| UX-AC-021 / US-005, US-007 | `AUTH-OK`, Translation initial state | Enter whitespace, advance 5 seconds; then choose each target option | Zero operations for whitespace; source counter remains canonical 0; target offers exactly four languages and no Translate button exists. |
| UX-AC-022 / US-006 | Target Romanian, `T-OK-A` | Fill source, advance 999 ms, then 1 ms; resolve | 0 then 1 request; UX-MSG-003; complete Romanian fixture replaces placeholder; UX-MSG-005; source focus/value unchanged; usage 7,543. |
...
| UX-AC-028 / US-005, US-007 | Manual source English, target English | Make source eligible | UX-MSG-007 with English; target invalid; zero operation. Select Romanian and advance 1 second → one request. |
...
| UX-AC-049 / US-011 | Prior state; result caret in changed sentence | Activate Sentence actions → Compare with source | Panel heading/Original/Rewritten visible; deleted spans struck through and programmatically described; focus first panel control; Escape returns to sentence trigger/caret. |
...
| UX-AC-052 / US-012 | Current result, selected sentence length 42, no cache, `ALT-OK` | Activate Generate alternatives; resolve | One alternatives request; loading text/skeletons then exactly 3 option buttons; usage +42 once; focus first option for keyboard activation. |
...
| UX-AC-054 / US-012 | Cached options and chosen alternative | Reopen same sentence | `Show alternatives`; same 3 options and selected state; request count and usage unchanged. |
...
| UX-AC-058 / US-013 | Full rewrite A pending, result editable | Change characters inside sentence 1 | UX-MSG-022; source unchanged; no request/charge; sentence 1 comparison/cache invalid; unaffected sentence metadata intact. |
...
| UX-AC-066 / US-014 | Successful full translation of 43 then rewrite of 80 | Resolve responses | Usage is replaced by each authoritative value; no optimistic increment; shared meter reflects both operations. |
...
| ID / story | Given | When | Then |
| --- | --- | --- | --- |
| UX-AC-088 / US-005 | Rewrite uses Detect automatically and uncertain-detection fixture | Enter eligible-length ambiguous text; advance clock | UX-MSG-006; zero provider operations/charge. Select a supported source and advance 1 second → one request if server validation accepts content. |
...
### 12.9 Visual regression suite

Use fixed system-font substitution assets, device scale factor 1, disabled caret, fixed time, reduced motion, deterministic scrollbars, and fixture output—never live provider text. Baselines are required for:
...
| --- | --- | --- | --- | --- |
| FR-001 | Routes `/login`, `/register`; AuthForm | US-002, US-003 | AC-006–007, 011–016 | Mocked UI; live Google/local integration in #6 |
| FR-002 | `/verify-email`, recovery routes | US-002, US-004 | AC-008–010, 017–020 | Mocked UI; email/token integration in #5/#6 |
| FR-003 | IA, ModeNav | US-001 | AC-001–005 | UI routes/history |
| FR-004 | LanguageSelect; validation states | US-005, US-007 | AC-021, 027–029 | UI fixtures + API validation contract |
...
| FR-007 | Counters, translation boundary, rewrite block | US-007, US-010 | AC-026, 030–032, 045 | UI + authoritative count integration (#5) |
...
| FR-013 | Mandatory correction copy/mode | US-009 | AC-038–042 | UI payload intent; quality in #4/#6 |
| FR-014 | Exclusive list/defaults | US-009 | AC-038–040 | UI selection/request contract |
| FR-015 | Correction-only override/restoration | US-009 | AC-041–042 | UI; restoration resolves Q-002 presentation |
| FR-016 | Rewrite timing/IME | US-009, US-010 | AC-039–044 | Controlled clock/events |
| FR-017 | Source protection | US-010–US-013 | AC-044–047, 052–064 | DOM value/caret assertions |
| FR-018 | Previous/current/failed result | US-010, US-013, US-015 | AC-044, 046–047, 058–060, 072, 076 | Response-order UI; reliability integration |
| FR-019 | ChangedText, ComparisonPanel, Copy | US-011 | AC-048–051 | UI/clipboard/accessibility |
| FR-020 | SentenceMenu/AlternativesPanel | US-012 | AC-052, 055–056 | UI fixture; 2–4/distinct/context quality in #6 |
| FR-021 | Alternative select/cache | US-012 | AC-053–054, 057 | Request-count and text assertions |
| FR-022 | Mode/source invalidation/full replacement | US-010, US-012, US-013 | AC-044, 046, 057, 060 | Controlled races |
| FR-023 | Result editing/copy/targeted invalidation | US-011, US-013 | AC-051, 058–064 | UI fixture identities; correspondence mechanics in #3/#5 |
| FR-024 | Counter/usage/truncated charge disclosure | US-007, US-014 | AC-030–032, 065–070 | UI + accounting integration |
| FR-025 | Alternatives charge once | US-012, US-014 | AC-052–056, 067 | UI request count + server accounting integration |
| FR-026 | Success-only/stale accounting | US-008, US-013–US-015 | AC-032, 035–036, 047, 055–061, 067, 072–076 | UI outcomes + atomic accounting integration |
| FR-027 | User/global limit/reset | US-014 | AC-068–070 | Visible UI + concurrent server tests in #6 |
| FR-028 | Usage disclosure | US-014 | AC-065–071 | UI authoritative replacement + API fields #5 |
...
| FR-033 | Eligible fallback triggers | Section 9.2 | US-015 | AC-067, 072–076 | Final UI outcome; fallback classification/integration #4/#6 |
| FR-034 | Final success/failure | Sections 6.2, 9.2 | US-015 | AC-036, 056, 072–076 | UI preservation + provider/fallback integration |
| FR-035 | Sentence/revision association | Sentence state tables | US-012, US-013 | AC-052–064 | UI with opaque fixture keys; representation/lifecycle #5 |
| FR-036 | Independent API capability | N/A | N/A | API-only #5/#6; SPA must not be part of API consumer test |
| FR-037 | Error presentation | Message catalog/auth/availability | US-002–US-004, US-007, US-014, US-015 | AC-007–020, 027–032, 068–076 | UI categories; schemas #5 |
| FR-038 | Defaults/session-only settings/English copy | Sections 3.4, 8.1 | US-001, US-009, US-017 | AC-001–003, 038, 042, 084–087 | UI/storage + teardown integration |
...
| NFR-005 | Accessibility/responsive sections | US-016 | AC-077–083 | Automated + required manual AT matrix |
...
| RG-002 | Quality | Result surfaces | N/A | Document #6 evaluation only |
| RG-003 | Performance | Loading/deadline states | US-015 | AC-073, 075 | UI state only; benchmark #6 |
| RG-004 | Accounting/recovery | Usage and race behavior | US-007, US-008, US-012–US-015 | AC-030–036, 052–076 | UI + integrated accounting suite |
| RG-005 | Access/integration | Auth stories | US-002–US-004 | AC-006–020 | UI auth plus independent API consumer #5/#6 |
| RG-006 | Web access | Accessibility story | US-016 | AC-077–083 | Browser matrix + manual AT |
| RG-007 | Privacy/cost | Workspace/availability | US-014, US-017 | AC-070, 084–087 | UI plus architecture/provider evidence |
| RG-008 | Specification readiness | Sections 15–16 | N/A | This document resolves UX questions only; other owners remain |
