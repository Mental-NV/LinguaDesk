...
**Version:** 1.7 · **Updated:** September 9, 2026
...
| 5 | **API behavioral design and generated contract** — `docs/05-api-design.md`; `docs/05-openapi.yaml` generated during implementation | The design owns shared authentication, counting, identity/lifecycle, recovery, usage and error semantics under Q-003/Q-006 with architecture. Selected delivery specifications refine operations and acceptance. C# DTOs/endpoint metadata supply the editable wire structure; generated OpenAPI is the reviewed machine-readable artifact. Use the authoring rules below; do not duplicate the complete schema in Markdown. |
| 6 | **Verification and LLM evaluation plan** — `docs/06-verification-plan.md` | Canonical test-layer allocation, harness/fixture strategy, execution gates, browser/visual/manual verification, LLM corpus/rubric/human review, workloads and reproducible evidence. Own Q-005 and the product/scenario coverage matrix. Reference behavior, design constraints and product thresholds from their owners rather than redefining them. |
...
**API contract authoring — amended with user approval on 2026-09-08:** Design shared behavior first in `docs/05-api-design.md`. Before implementing an API slice, resolve its applicable questions and specify operations, inputs/outputs and observable acceptance in its selected `spec.md`; update the shared design when a decision affects other slices. Do not scaffold a backend solely to finish the numbered planning documents, and do not handwrite a provisional `docs/05-openapi.yaml`.

At the start of each selected API implementation slice, add its actual C# DTOs, endpoint metadata and semantic descriptions/transformers, then deterministically generate and review `docs/05-openapi.yaml` against the shared design and selected specification. Generate client types before dependent clients consume that shape. Handlers are implemented against the reviewed shape. The generation milestone is an early implementation gate, not a reason to postpone contract review until handlers are finished. Shared design and selected acceptance remain authoritative for intended behavior; generation cannot authorize an accidental behavior change.

Keep one editable source for wire structure in C# and one committed generated schema artifact. Do not hand-edit YAML or maintain a competing full schema in Markdown. Client-facing descriptions/examples belong in C# documentation/metadata and flow into OpenAPI; the behavioral design retains cross-operation rules and acceptance examples. The first generated document covers the selected implemented/implementing surface, with incomplete slice status explicit; it must not imply that all MVP capabilities or documented handlers work. Contract-only endpoints under construction must not ship as working APIs. CI regenerates schema/client and fails on drift, and separate behavioral tests verify authentication, accounting, recovery and errors. P-006 compatibility guarantees remain proposed. ADR-004 records the rationale.
...
**Milestone sizing and revision — user direction, 2026-09-09:** #7 stays at user-story, feature or necessary-enablement level, with basic infrastructure first. Concrete backlog items, selected specifications, implementation plans and tasks belong to #8 as a milestone approaches execution. Keep each milestone focused on a coherent, independently verifiable outcome. The previous 30-minute limit is removed: there is no fixed elapsed-time cap or model-specific duration gate. Assess scope, dependencies and verification effort before execution; split work where that improves cohesion or manages risk, without weakening acceptance or verification. Keep larger release outcomes and human/live-service evidence gates distinct from executable milestones. Stable milestone IDs express identity, not order; reorder or split future scope with explicit dependency and history updates, preserving completed evidence and the PRD's scope authority.
...
The split between backlog selection and delivery packages is a LinguaDesk convention. The package names and separation of specification, planning, and tasks follow [GitHub Spec Kit's artifact approach](https://github.com/github/spec-kit/tree/main/templates). Shared API behavior remains canonical in document #5's design. Package-level material specifies the selected operation behavior and links to shared rules; once generated, the OpenAPI artifact supplies wire details rather than a second package-level schema.
...
| **API contract ready for dependent clients** | During a selected API implementation slice, actual C# contracts generate reviewed OpenAPI/client types that agree with the behavioral design and selected acceptance. This is required before client adoption, but is not evidence that handlers work. The absence of YAML before the first such slice does not itself block planning or unrelated implementation. |
